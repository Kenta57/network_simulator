...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  // m_highTxMarkはReTxにかかわらず, これまでの最大のseq no.のこと. したがって送り出すseqが最高値でないということは一度送信しているということ.
  ...
  if (!m_history.empty ())
    {
      ...
      // retxは再送しているかどうかのflag, seqは送信されたパケットの最初のシーケンス番号, countは送信されたバイト数
      // hはACKを受け取るより前に送出されたセグメントである. 複数のsegmentの確認応答を一度にすると等号が成立しない
      // 受信したACKに対応する送出したセグメントだけを見たい. そうでなければスルー？
      // 複数の送信したsegmentに対して, 1つのACKが返ってくることが一般的であり, その時は初めに送ったもののsegmentを基準にRTTのサンプルを取る？
      if (!h.retx && ackSeq >= (h.seq + SequenceNumber32 (h.count)))
        { // Ok to use this sample
          // m_timestampEnabledはTimeStampオプションが有効化どうか, tcpHeader.HasOption(uint8_t kind)はkindのオプションがヘッダに含まれているかどうか
          if (m_timestampEnabled && tcpHeader.HasOption (TcpOption::TS))
            {
              ...
              // ヘッダー内のTimestamp Echo Replyの値を取得し, それをもとに送出して返ってくるまでの時間を計算
              // 内部的にTimestampをカウントしており, それをもとにTSecrの値と今のTSの値から経過時間を計算する
              m = TcpOptionTS::ElapsedTimeFromTsValue (ts->GetEcho ());
            }
          else
            {
              // 単純にhのtimeと現時刻を比べると実際の経過時間よりも長く見積もられる可能性がある. 
              // hとして取り出したものが必ずしもtcpHeaderに対応するものとは限らないため？
              // 処理するタイミングによって変わる可能性がある
              m = Simulator::Now () - h.time; // Elapsed time
            }
        }
    }

  // 一気にACKを返した分など使用しなかったヘッダの情報を削除する
  // Now delete all ack history with seq <= ack
  while (!m_history.empty ())
    {
      RttHistory& h = m_history.front ();
      // まだACKを受信していないセグメントに関してはm_historyに保存しておく
...
// TCPヘッダ内の情報をm_tcb(TCPSocketState)に移し替える関数？
void
                                       ...
                                       const SequenceNumber32 &seq)
{
  ...
  // m_timestampToEchoはRFC 7323でいうTS.Recent??
